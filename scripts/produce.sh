...
initFun(){
    ...
    if [[ "$appName" == "" ]];then
        echoFun "name is empty" err
        exit 1
    fi
    echoFun "name: $appName" tip

    appAddr=$(getJsonValue "http_addr")
    if [[ "$appAddr" == "" ]];then
        echoFun "http_addr is empty" err
        exit 1
    fi
    echoFun "http_addr: $appAddr" tip

    logDir=$(getJsonValue "log_dir")
    if [[ "$logDir" == "" ]];then
        echoFun "log_dir is empty" err
        exit 1
    fi
    echoFun "log_dir: $logDir" tip
}
